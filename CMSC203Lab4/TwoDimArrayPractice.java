...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void setValues( int value, int row )
   {
    ...
    //  and column is the index of the array's current column
    // Write your code here:
	int numColumns = intArray[row].length;
    for (int column = 0; column < numColumns; column++) {
        intArray[row][column] = value;

           // Animate the algorithm by calling animate() at the end of each iteration
        animate(row, column);
        }
   }
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   ...
	   /*variable with the maximum possible value for an int data type in Java.*/
	   int minimum = Integer.MAX_VALUE; 
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        ...
	        animate(row, column,  minimum); // Animate the algorithm
	    }
